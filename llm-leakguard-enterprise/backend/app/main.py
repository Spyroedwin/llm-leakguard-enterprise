...
# ===== PHASE 1: TOR + GEO MIDDLEWARE ===== (YOUR ORIGINAL - UNCHANGED)
...
# ===== PHASE 1: TRADITIONAL THREAT SCANNER ===== (YOUR ORIGINAL - UNCHANGED)
threat_patterns = {
...
# ===== PHASE 2: OWASP LLM TOP 10 SCANNER ===== (YOUR ORIGINAL - UNCHANGED)
...
# Initialize scanners (YOUR ORIGINAL - UNCHANGED)
...
# ===== NEW: PHASE 4 HONEYPOT MIDDLEWARE =====
@app.middleware("http")
async def honeypot_middleware(request: Request, call_next):
    path = request.url.path
    client_ip = request.state.geo["ip"] if hasattr(request.state, 'geo') else "unknown"
    
    # Honeypot trap paths + tarpit
    if any(h in path for h in ['/admin', '/honeypot', '/pentbox']):
        logger.critical(f"🚨 HONEYPOT HIT: IP={client_ip}, Path={path}")
        await asyncio.sleep(10)  # Tarpit delay
    
    response = await call_next(request)
    return response

# Logging setup (YOUR ORIGINAL - ENHANCED)
os.makedirs("../logs", exist_ok=True)
logging.basicConfig(filename="../logs/leakguard.jsonl", level=logging.INFO, format="%(message)s")
...
async def get_logs(limit: int = 50):
    ...
    try:
        with open("../logs/leakguard.jsonl", "r") as f:
    ...
    if os.path.exists(pentbox_log):
        try:
            with open(pentbox_log, 'r', encoding='utf-8') as f:
                lines = f.readlines()
                logs = [{"timestamp": datetime.now().isoformat(), "line": line.strip()} 
                       for line in lines[-tail:] if line.strip()]
            
            # Auto-add critical PentBox alerts to main log
            for log_entry in logs:
                if any(keyword in log_entry["line"].upper() for keyword in ["INTRUSION", "DETECTED", "ATTACK"]):
                    alert_data = {
                        "ip": "pentbox_honeypot",
                        "threat_type": "PENTBOX_ALERT",
                        "severity": "CRITICAL",
                        "details": log_entry["line"][:200]
                    }
                    send_alert("🚨 PENTBOX HONEYPOT HIT", alert_data)
                    
        except Exception as e:
            logs = [{"timestamp": datetime.now().isoformat(), "line": f"PentBox log read error: {str(e)}"}]
    
    return {"honeypot_logs": logs, "total_alerts": len([l for l in logs if "INTRUSION" in l["line"]])}
